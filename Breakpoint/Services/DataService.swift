...
class DataService {
    ...
    func getAllMessagesFor(desiredGroup: Group, handler: @escaping (_ messagesArray: [Message]) -> ()) {
        ...
        getGroupsUrl().child(desiredGroup.key).child("messages").observeSingleEvent(of: .value) { (groupMessageSnapshot) in
    ...
    func getEmails(group: Group, handler: @escaping (_ emails: [String]) -> ()) {
        var emailArray = [String]()
        getUsersUrl().observeSingleEvent(of: .value) { (userSnapshot) in
            guard let userSnapshot = userSnapshot.children.allObjects as? [DataSnapshot] else { return }
